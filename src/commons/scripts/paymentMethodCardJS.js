...
function appendPaymentMethodCard(cardInfo) {
    ...
    paymentCard.setAttribute('data-payment-id', cardInfo.paymentId);
...
function appendPaymentMethodPaypal(paypalInfo) {
    let paymentCard = document.createElement('div');
    buyerProfileInforCardClasses(paymentCard);
    paymentCard.classList.add('payment-info-card');
    paymentCard.setAttribute('data-payment-id', paypalInfo.paymentId);
    ...
    <div class="flex flex-row mt-5">
        <div class="flex flex-row">
            <p class="card-info-text">Tipo:</p><p id="number-info-display" class="card-info-display">${paypalInfo.type}</p>
        </div>
        <div class="flex flex-row ml-6">
            <p class="card-info-text">Titular:</p><p id="name-info-display" class="card-info-display">${paypalInfo.holder}</p>
        </div>
    </div>
    <div class="flex flex-row mt-5">
        <p class="card-info-text">Correo:</p><p id="type-info-display" class="card-info-display">${paypalInfo.cardNumber}</p>
    </div>
    `;
