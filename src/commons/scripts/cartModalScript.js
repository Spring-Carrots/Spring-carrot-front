...
function cartModalCard(listing) {
    ...
    listingElement.id = `listing-${listing.id}-card`;
    cartModalCardClasses(listingElement);

    listingElement.innerHTML = `
        <div class="object-cover rounded-l-lg col-span-1" style="object-position: 50% 50%">
          <img class="rounded-l-lg" src="${listing.foto1}">
        </div>
        <div class="mt-2 mb-2 grid grid-cols-3 col-span-3">
           <div class="flex flex-col col-span-2">
              <h6 class="font-bold">${listing.nombre}</h6>
              <p class="truncate-lines-2">${listing.descripcion}</p>
            </div>
            <div class="flex flex-row items-center ml-1 mr-2">
              <input type="number" class="cart-listing block border-0 w-10 h-6" min="0" value="${listing.precio}">
              <button class="inline-block ml-2 justify-center items-center w-8 h-8 rounded-lg cart-modal-listing-delete" data-listing-id="${listing.id}" style="background-color: #f50538">
                <i class="fa-solid fa-trash-can cart-modal-listing-trash-can" data-listing-id="${listing.id}" style="color: #ffffff;"></i>
              </button>
            </div>
        </div>
    `;
